...
const Portfolio = ({ handleProjectClick }) => {
	...
	return (
		<div id='portfolio' className='flex flex-col  w-full  h-full  lg:px-8 xl:px-10 py-8'>
			...
			<section className='relative w-full space-y-20 px-4 md:px-8 lg:px-32 xl:px-20 2xl:px-32 3xl:px-60'>
				{data.map((item, index) => (
					<div data-aos='fade-up' data-aos-duration='500' data-aos-once='true' key={item.id} className={`grid md:grid-cols-2 gap-4 md:gap-8 items-start  rounded-lg p-2  `}>
						<div className={`shadow-xl relative block h-[150px] md:h-[250px]  w-full   ${index % 2 !== 0 ? 'md:order-1' : 'md:-order-1'}`}>
							<Image src={item?.image} alt='image' layout='fill' className='rounded-md aspect-auto ' />
						</div>
